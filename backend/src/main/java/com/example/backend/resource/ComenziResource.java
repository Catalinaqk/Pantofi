...
public class ComenziResource {
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<Comenzi> getComenziById(@PathVariable(value = "id") String id) {
        return ResponseEntity.ok().body(comenziService.getComenziById(id));
    }

    /**
     * Deletes a comenzi by its unique identifier.
     *
     * @param id the unique identifier of the comenzi.
     * @return the ResponseEntity with status 204 (NO_CONTENT).
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<String> deleteComenzi(@PathVariable(value = "id") String id) {
        comenziService.deleteComenzi(id);
        return ResponseEntity.ok().body("Comenzi was deleted.");
    }
}